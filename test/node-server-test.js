...
describe("Manifest", function() {
	...
	it("should generate a manifest based on an input", function(done)
	{
		...
		var input = { "sounds": [{"md5":"ImASound.mp3"}], "costumes": [{"baseLayerMD5":"ImACostume.jpg"}], "children":
			...
			manifest.generateManifest("TestManifest", input, function(data)
			{
				// Store the manifest data, run a quick check to see if it exists.
				manifestData = data;
				expect(manifestData).not.toBeUndefined();
				expect(data.indexOf("CACHE MANIFEST")).not.toBe(-1);
				done();
			});
	});
	...
	it("should generate the correct manifest based on that input", function() {
		...
		expect(manifestData.indexOf("projects/TestManifest/get/")).not.toBe(-1);
		expect(manifestData.indexOf("asset/ImASound.mp3/get/")).not.toBe(-1);
		expect(manifestData.indexOf("asset/ImACostume.jpg/get/")).not.toBe(-1);
		expect(manifestData.indexOf("asset/ImASound2.mp3/get/")).not.toBe(-1);
		expect(manifestData.indexOf("asset/ImACostume3.jpg/get/")).not.toBe(-1);
	})
	...
	it("should not have duplicate entries in the manifest", function() {
		...
		manifestData = manifestData.replace("projects/TestManifest/get/","");
		manifestData = manifestData.replace("asset/ImASound.mp3/get/","");
		manifestData = manifestData.replace("asset/ImACostume.jpg/get/","");
		manifestData = manifestData.replace("asset/ImASound2.mp3/get/","");
		manifestData = manifestData.replace("asset/ImACostume3.jpg/get/","");


		expect(manifestData.indexOf("projects/TestManifest/get/")).toBe(-1);
		expect(manifestData.indexOf("asset/ImASound.mp3/get/")).toBe(-1);
		expect(manifestData.indexOf("asset/ImACostume.jpg/get/")).toBe(-1);
		expect(manifestData.indexOf("asset/ImASound2.mp3/get/")).toBe(-1);
